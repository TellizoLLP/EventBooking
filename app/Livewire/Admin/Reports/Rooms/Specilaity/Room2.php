...
use Excel;
use App\Exports\Specilaity\Room2Report;
...
class Room2 extends Component
{
    ...
    {
        $query = EventRegistrationSession::where('room_id', 2)->where('course_id', 1)
        ...
        $this->parents = EventRegistrationSession::where('room_id', 2)
        ->where('course_id', 1)
            ->whereRelation('eventRegistration', 'current_status', 2)
            ->count();

        $this->total = $this->students + $this->parents;
        return view('livewire.admin.reports.rooms.specilaity.room2');
    }
     /* export to excel */
