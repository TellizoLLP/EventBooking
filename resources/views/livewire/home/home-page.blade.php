<div class="h-screen w-screen flex flex-col overflow-clip" x-data="alpineSystem">
    <div class="h-[calc(100vh-2rem)] overflow-y-auto w-full">
        ...
        <div class="w-full">
            ...
            <div class="p-6 bg-gray-100  space-y-6 relative  h-full w-full overflow-y-auto">
                <div class="bg-white p-6 rounded-lg shadow" x-show="page == 1"
                    ...
                    <div class="space-y-4">
                        <div class="w-full grid sm:grid-cols-1 lg:grid-cols-2 gap-4">
                            ...
                            @endif
                            <div class="col-span-2 sm:col-span-1">
                                <label class="block text-sm font-medium text-gray-600 mb-1">How did you hear about us?
                            ...
                            <div class="flex justify-end">
                                <button @click="$wire.pageOneSave()"
                                    class="px-5 py-2.5 h-fit  cursor-pointer text-sm antialiased font-semibold bg-[#285a49] text-white rounded-lg">
                                    Next
                                </button>
                            </div>
                        </div>
                ...
                <div class="bg-white p-4 rounded-lg shadow" x-show="page == 2"
                    x-transition:enter="transition-opacity transform ease-out duration-500"
                    x-transition:enter-start="opacity-0 -translate-x-full"
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition-opacity transform ease-in duration-500 absolute inset-0"
                    ...
                    <div class="flex w-full justify-between items-center">
                        <div class="flex flex-col">
                            ...
                            <div class="text-xs font-medium text-gray-600 mb-4 antialiased">You can choose only 3 of
                                the 5
                                Medzone
                                experiences
                                below</div>
                        ...
                        @foreach ($rooms as $roomIndex => $room)
                            <div class="">
                                <div class="flex gap-2 bg-gray-100 w-fit px-3 py-2 rounded-t-lg mt-3.5">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-6 text-black"
                                        viewBox="0 0 24 24">
        ...
        {{ !$session['clickable'] || in_array($session['id'], $disabledTimeSlots) || ($session['slots'] != 0 && $slotsinline['filled'] >= 40) ? 'opacity-50 cursor-not-allowed bg-gray-300' : '' }}
                                                        ...
                                                        @if ($room['roomName'] === 'Studio 7')
                                                            <div
                                                                ...
                                                                <svg xmlns="http://www.w3.org/2000/svg" class="size-6"
                                                                    viewBox="0 0 24 24">
                                                                    <path fill="currentColor"
                                                                        d="M16 14h-3v3h-2v-3H8v-2h3V9h2v3h3m5-7h-2.65l1.15-3.15L17.15 1l-1.46 4H3v2l2 6l-2 6v2h18v-2l-2-6l2-6z" />
                                                                </svg>
                                                            </div>
                                                        @endif
                                                        @if ($room['roomName'] === 'Room 6')
                                                            <div
                                                                class="w-10 h-10 bg-green-400 rounded-full flex items-center justify-center">
                    ...
                    <div class="">
                        <div class="flex w-full justify-between items-center">
                            <div class="flex flex-col">
                                ...
                                <div class="text-xs font-medium text-gray-600 mb-4 antialiased">You can choose only 1
                                    Micro-courses
                                    below</div>
                            </div>
                            <div class="flex gap-2">
            ...
            {{ $mainSession['slots'] != 0 && $slots['filled'] >= 40 ? 'opacity-50 bg-gray-300 cursor-not-allowed' : '' }}"
                                                ...
                                                <div class="flex items-center justify-between">
                                                    <div class="flex items-center">
                                                        <div>
                                                            <p class=" font-semibold">
                                                                {{ $mainSession['name'] }} {{ $slots['filled'] }}</p>
                                                            <p class="text-sm">
                                                                {{ $mainroom['roomName'] }}</p>
                                                            <span
                                                                class="text-xs">({{ $mainSession['session'] }})</span>
                                                        </div>
                                                    </div>
                                                </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow" x-show="page == 5"
                x-transition:enter="transition-opacity transform ease-out duration-500"
                x-transition:enter-start="opacity-0 -translate-x-full"
                x-transition:enter-end="opacity-100 translate-x-0"
                x-transition:leave="transition-opacity transform ease-in duration-500 absolute inset-0"
                ...
                <div class="">
                    <div class="flex w-full justify-between items-center">
                        <div class="flex flex-col">
                            <h2 class="text-lg font-semibold text-gray-800 antialiased">Additional Sessions</h2>
                            <div class="text-xs font-medium text-gray-600 mb-4 antialiased">You can any sessions
                                below</div>
                        </div>
                        <div class="flex gap-2">
                    ...
                    <div class="flex flex-col">
                        ...
                        <div class="flex bg-gray-100 p-2 py-3">
                            <div class="w-full grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-2">
            ...
            w-full rounded-lg shadow p-4"
                                            ...
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center">
                                                    <div>
                                                        <p class="font-semibold">
                                                            {{ $additionalSession['name'] }}</p>
                                                        <p class="text-sm">
                                                            {{ $additionalroom['roomName'] }}</p>
                                                        <span
                                                            class="text-xs">({{ $additionalSession['session'] }})</span>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div class="col-span-2 flex flex-col gap-2">
                                ...
                                @foreach ($items as $item)
                                    @php
                                        ...
                                        $selectedSession = collect($rooms[$roomIndex]['sessions'])->firstWhere(
                                            'id',
                                            $item->session_id,
                                        );
                                    ...
                                    @if ($sessionDetails)
                                        <div class="p-4 border rounded-lg shadow flex flex-col gap-2">
                                            ...
                                            <p><strong>Time:</strong> {{ $sessionDetails['startTime'] }} -
                                                {{ $sessionDetails['endTime'] }}</p>
                                        </div>
                                    @endif
                                @endforeach
                            </div>
                        ...
                        @if ($itemsMicro->count() > 0)
                            <div class="col-span-2">
                                ...
                                @foreach ($itemsMicro as $item)
                                    @php
                                        $roomIndex = $item->room_id;
                                        $selectedSession = collect($Mainrooms[$roomIndex]['sessions'])->firstWhere(
                                            'id',
                                            $item->session_id,
                                        );
                        ...
                        @if ($itemsAdditional->count() > 0)
                            <div class="col-span-2">
                                ...
                                @foreach ($itemsAdditional as $item)
                                    @php
                                        $roomIndex = $item->room_id;
                                        $selectedSession = collect(
                                            $Additionalrooms[$roomIndex]['sessions'],
                                        )->firstWhere('id', $item->session_id);
                                        // $selectedSession = $Additionalrooms[$roomIndex]['sessions'][$item->session_id] ?? null; // Safe fetch for session data
                                        $sessionDetails = $selectedSession
                                            ? [
                                                'roomName' =>
                                                    $Additionalrooms[$roomIndex]['roomName'] ?? 'Unknown Room',
                                                'name' => $selectedSession['name'] ?? 'Unknown Session',
                                                'sessionName' => $selectedSession['session'] ?? 'Unknown Session',
                                                'startTime' => $selectedSession['start_time'] ?? 'N/A',
                                                'endTime' => $selectedSession['end_time'] ?? 'N/A',
